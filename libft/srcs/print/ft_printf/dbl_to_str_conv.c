									...
									int exponent)
{
	...
	if (exponent > 0)
	{
		ft_bigint_pow10(&pow10, exponent);
		ft_bigint_multiply(val_den, &pow10, &bigint_tmp);
		*val_den = (t_bigint){0, {0}};
		ft_bigint_cpy(val_den, &bigint_tmp);
	}
	else if (exponent < 0)
	{
		ft_bigint_pow10(&pow10, -exponent);
		ft_bigint_multiply(val_num, &pow10, &bigint_tmp);
		*val_num = (t_bigint){0, {0}};
		ft_bigint_cpy(val_num, &bigint_tmp);
	}
}
							...
							t_format *conv_params)
{
	...
	while (val_num.length > 0 && i < BUF_DIGITS_SIZE)
	{
		...
		digits[i] = '0' + digit;
		bigint_tmp = (t_bigint){0, {0}};
		ft_bigint_cpy(&bigint_tmp, &val_den);
